...
flowchart TD
    ...
    E -->|Failure| G[Update State → Failed and Schedule Retry]
    G --> H{Max Retries Reached?}
    H -->|No| I[Calculate Backoff Delay and Retry Later]
    H -->|Yes| J[Move to DLQ - Dead Letter Queue]
    F --> K[CLI List or Web Dashboard]
    I --> C
    J --> K
